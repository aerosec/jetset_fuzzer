...
#define AFL_QEMU_CPU_SNIPPET                                                   \
  do {                                                                         \
    if (itb->pc == afl_entry_point && !afl_setup_done) {                       \
      afl_setup();                                                             \
      afl_forkserver(cpu);                                                     \
    }                                                                          \
...
/* Directory in which to store the checkpointed processes for forking */
char *afl_criu_dir;
char *afl_fuzzer_name;
char *afl_criu_state_fns = 0;

int afl_setup_done = 0;
/* variable to determine whether the current proc is fs */
...
/* We only checkpoint the forkserver once */
int criu_dump_done = 0;
// Whether we told AFL to start the timer
...
static inline void afl_maybe_log(ulong);

static void afl_wait_tsl(CPUState *, int);
static void afl_request_tsl(target_ulong, target_ulong, target_ulong, uint64_t);
 ...
 */
static void criu_copy_file_state(void) {
...
static void criu_restore_file_state(void) {
  if (afl_criu_state_fns) {
...
static inline void afl_maybe_log(ulong cur_loc) {
#ifndef VALIDATING_AFL
