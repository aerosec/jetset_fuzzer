...
static u8 run_target(char** argv, u32 timeout) {
  ...
  if (dumb_mode == 1 || no_forkserver) {
    ...
    fprintf(stderr, "AFL GOT CHILD ID: %d\n", child_pid);

    if (child_pid <= 0) FATAL("Fork server is misbehaving (OOM?)");

  }

  // Wait from restore to start timer
  ...
  if (dumb_mode == 1 || no_forkserver) {
    ...
    if ((res = read(fsrv_st_fd, &status, 4)) != 4) {
      ...
      fprintf(stderr, "No fork server status response. %s\n", strerror(errno));
...
static u8 save_if_interesting(char** argv, void* mem, u32 len, u8 fault) {
  ...
  switch (fault) {
    ...
    case FAULT_TMOUT:
      ...
      fn = alloc_printf("%s/hangs/num:%ld,id:%06llu,%s", out_dir,
                        number_fuzzed, unique_hangs, describe_op(0));
...
EXP_ST u8 common_fuzz_stuff(char** argv, u8* out_buf, u32 len) {
  ...
  // Create test case output file, may be deleted later
  fault = run_target(argv, exec_tmout);
...
static void handle_timeout(int sig) {
  if (child_pid > 0) {
    ...
    fprintf(stderr, "AFL: Child timed out, killing %d\n", child_pid);
    child_timed_out = 1;
    kill(child_pid, SIGKILL);

  } else if (child_pid == -1 && forksrv_pid > 0) {

    fprintf(stderr, "AFL: no child pid, killing FS %d\n", forksrv_pid);
    child_timed_out = 1;
...
static char** get_qemu_argv(u8* own_loc, char** argv, int argc) {
  ...
  /* NOTE: Memory leaks, whatever */

  char** new_argv = ck_alloc(sizeof(char*) * (argc + 4));
  u8 *tmp, *cp, *rsl, *own_copy;

  /* Create the output file for child processes of this fuzzer */
...
int main(int argc, char** argv) {
  ...
  while ((opt = getopt(argc, argv, "+i:o:f:m:t:T:dnCB:S:M:x:Qc:")) > 0)
    ...
    switch (opt) {
      ...
      case 'c': /* Which cpu to pin to */
          cpu_pin_to = atoi(optarg);
          break;
